...
class UPSTrapReceiver:
        ...
        try:
                ...
                if input_thread.is_alive():
                    ...
                    if self.panel_led_controller:
                        try:
                            ...
                            # Update config.py: LED 10 enabled -> ALARM_STATUS = True
            ...
            if self.panel_led_controller:
                try:
                    ...
                    # Priority 1: Check if both sources fail OR timeout (this overrides everything)
                    # Note: Timeout is already handled above, but we check here for both sources fail
                    if (source_a_status_lower == 'fail' and source_b_status_lower == 'fail'):
                        # Both sources fail: disable LEDs 2,3,4,6,7,8,9,11,12,13,14 and enable LED 10
                        leds_to_disable = [2, 3, 4, 6, 7, 8, 9, 11, 12, 13, 14]
                        for led in leds_to_disable:
                            self.panel_led_controller.disable_led(led)
                        # Enable LED 10 (alarm LED)
                        self.panel_led_controller.enable_led(10)
                        self.logger.info(f"Both sources fail - Disabled LEDs {leds_to_disable}, Enabled LED 10")
                        # Update config.py: LED 10 enabled -> ALARM_STATUS = True
                        self._update_alarm_status_config(True)
                    else:
                        # Priority 2: Control LEDs based on Source A Status
                        ...
                        # Priority 3: Control LEDs based on Source B Status
                        ...
                        # Priority 4: Control LED 3, 8, and 10 based on combined Source A and Source B status
                        if source_a_status_lower == 'ok' and source_b_status_lower == 'ok':
                            ...
                            # Update config.py: LED 10 disabled -> ALARM_STATUS = False, BUZZER_MUTED = False
                            self._update_alarm_status_config(False)
                            self._update_buzzer_muted_config(False)
                            # Update internal state
                            self.buzzer_muted = False
                            self.alarm_status = False
                        elif source_a_status_lower == 'fail' or source_b_status_lower == 'fail':
                            # At least one source is fail: disable LED 3
                            self.panel_led_controller.disable_led(3)
                            self.logger.debug(f"At least one source fail - Disabled LED 3")
                        
                        # Priority 5: Control LEDs based on Output Source
                        ...
                        # Priority 6: Control LEDs based on Output Load percentage (using config.py thresholds)
                        ...
                        if output_load_percent is not None:
                            try:
                                ...
                                # L1: LED 14 (Low load) - between L1_LOAD_MIN and L1_LOAD_MAX
                                if self.l1_load_min <= load_int <= self.l1_load_max:
                                    # L1 range: enable LED 14, disable LED 11, 12, 13
                                ...
                                # L2: LED 13, 14 (Medium load) - between L2_LOAD_MIN and L2_LOAD_MAX
                                elif self.l2_load_min <= load_int <= self.l2_load_max:
                                    # L2 range: enable LED 13, 14, disable LED 11, 12
                                ...
                                # L3: LED 12, 13, 14 (Medium-high load) - between L3_LOAD_MIN and L3_LOAD_MAX
                                elif self.l3_load_min <= load_int <= self.l3_load_max:
                                    # L3 range: enable LED 12, 13, 14, disable LED 11
                                ...
                                # L4: LED 11, 12, 13, 14 (Overload) - >= L4_LOAD_THRESHOLD
                                elif load_int >= self.l4_load_threshold:
                                    # L4 range: enable LED 11, 12, 13, 14
                                    ...
                                    # Load outside all ranges: all LEDs off (safety fallback)
                                    self.panel_led_controller.disable_led(14)
                                    self.panel_led_controller.disable_led(13)
                                    self.panel_led_controller.disable_led(12)
                                    self.panel_led_controller.disable_led(11)
                                    self.logger.debug(f"Load {load_int}%: All LEDs OFF (outside valid range: L1={self.l1_load_min}-{self.l1_load_max}%, L2={self.l2_load_min}-{self.l2_load_max}%, L3={self.l3_load_min}-{self.l3_load_max}%, L4>={self.l4_load_threshold}%)")
                            except (ValueError, TypeError) as e:
                                self.logger.warning(f"Could not parse load percentage '{output_load_percent}': {e}")
                        else:
                            self.logger.warning(f"output_load_percent is None - cannot control load-based LEDs. output_load='{output_load}', output_status keys: {list(output_status.keys()) if output_status and isinstance(output_status, dict) else 'N/A'}")
                    
                    # Priority 7: Control buzzer based on LED 10, LED 11, and LED 8 states
                    # This runs after all LED control (both if and else branches) to ensure buzzer state matches LED states
                    try:
                        ...
                        # - Otherwise, disable buzzer (safety)
                        ...
                        if led_10_state is True or led_11_state is True:
                            ...
                            # Other cases: disable buzzer (LED 10 not enabled, LED 11 not enabled, and LED 8 not enabled)
                            if hasattr(self.panel_led_controller, 'disable_buzzer'):
                                self.panel_led_controller.disable_buzzer()
                                self.logger.debug(f"Buzzer disabled (LED 10={led_10_state}, LED 11={led_11_state}, LED 8={led_8_state})")
                        
                        # Update config.py based on LED 10 state
                        if led_10_state is True:
                            # LED 10 enabled -> ALARM_STATUS = True
                            self._update_alarm_status_config(True)
                        elif led_10_state is False:
                            # LED 10 disabled -> ALARM_STATUS = False, BUZZER_MUTED = False
                            self._update_alarm_status_config(False)
                            self._update_buzzer_muted_config(False)
                            # Update internal state
                            self.buzzer_muted = False
    ...
    def _reset_button_callback(self, channel: int):
        ...
        # Also log immediately to file
        try:
            self.logger.info(f"[BUTTON-CALLBACK] Reset button callback triggered on GPIO {channel}")
            for handler in self.logger.handlers:
                if isinstance(handler, logging.FileHandler):
                    handler.flush()
        except:
        ...
        try:
            ...
            # Read button state (LOW = pressed, HIGH = released)
            if not RPI_GPIO_AVAILABLE:
                self.logger.warning("[BUTTON-CALLBACK] RPi.GPIO not available in callback")
                return
            
            button_state = GPIO.input(channel)
            is_pressed = (button_state == GPIO.LOW)
            self.logger.info(f"[BUTTON-CALLBACK] GPIO {channel} read: state={button_state}, is_pressed={is_pressed}, LOW={GPIO.LOW}, HIGH={GPIO.HIGH}")
            ...
            # Log all button state changes for debugging (including releases)
            if is_pressed:
                self.logger.info(f"[BUTTON] Reset button state change detected: PRESSED (is_pressed={is_pressed}, button_state={button_state}, LOW={GPIO.LOW}, HIGH={GPIO.HIGH}, time_since_last={time_since_last:.3f}s)")
            else:
                self.logger.debug(f"[BUTTON] Reset button state change detected: RELEASED (is_pressed={is_pressed}, button_state={button_state}, LOW={GPIO.LOW}, HIGH={GPIO.HIGH}, time_since_last={time_since_last:.3f}s)")
            
            # Only process button press (LOW), not release (HIGH)
            if is_pressed:
                self.reset_button_last_change_time = current_time
                self.logger.info("=" * 70)
                self.logger.info("RESET BUTTON PRESSED - DETECTED!")
                self.logger.info(f"Button state: {button_state} (LOW={GPIO.LOW}, HIGH={GPIO.HIGH})")
                self.logger.info(f"Time since last callback: {time_since_last:.3f}s")
                self.logger.info("Triggering reset functionality...")
                self.logger.info("=" * 70)
                # Force log flush to ensure we see this immediately
                for handler in self.logger.handlers:
                    if isinstance(handler, logging.FileHandler):
                        handler.flush()
                # Perform reset in a separate thread to avoid blocking the callback
                reset_thread = threading.Thread(target=self._perform_reset, daemon=True)
                reset_thread.start()
            else:
                # Log button release for debugging
                self.logger.debug(f"Reset button released (state: {button_state}, is_pressed: {is_pressed})")
    ...
    def _send_snmp_reset_command(self, host: str, community: str = 'public', port: int = 161) -> bool:
        """
        Send SNMP SET command to reset the STS device.
        
        Args:
            host: UPS/STS IP address
        ...
        """
        # Reset OID (atsAgent=2, device firmware version)
        RESET_OID_V2 = '1.3.6.1.4.1.37662.1.2.2.1.1.7.12'  # agentConfigResetToDefault (atsAgent=2)
        RESET_OID_V3 = '1.3.6.1.4.1.37662.1.2.3.1.1.7.12'  # agentConfigResetToDefault (atsAgent=3, fallback)
        ACTION_EXECUTE = 1  # Value to execute reset
        
        # Try V2 first (device firmware version)
        reset_oid = RESET_OID_V2
        
        try:
            # Try using pysnmp entity API first
            try:
                from pysnmp.entity.rfc3413.oneliner import cmdgen
                self.logger.info(f"Sending SNMP reset command to {host} using pysnmp entity API")
                cmdGen = cmdgen.CommandGenerator()
                errorIndication, errorStatus, errorIndex, varBinds = cmdGen.setCmd(
                    cmdgen.CommunityData(community, mpModel=1),  # SNMPv2c
                    cmdgen.UdpTransportTarget((host, port)),
                    (reset_oid, rfc1902.Integer(ACTION_EXECUTE))
                )
                
                if errorIndication:
                    self.logger.error(f"SNMP reset error: {errorIndication}")
                    # Try V3 OID as fallback
                    if reset_oid == RESET_OID_V2:
                        self.logger.info(f"Trying alternative OID (atsAgent=3): {RESET_OID_V3}")
                        reset_oid = RESET_OID_V3
                        errorIndication, errorStatus, errorIndex, varBinds = cmdGen.setCmd(
                        ...
                        if errorIndication:
                            raise Exception(f"SNMP reset error with V3 OID: {errorIndication}")
                    else:
                        return False
                elif errorStatus:
                    ...
                    self.logger.error(f"SNMP reset error status: {error_msg}")
                    return False
                else:
                    # Success
                    for varBind in varBinds:
                        oid_str, val = varBind
                        self.logger.info(f"Reset command sent successfully: {oid_str} = {val}")
                    return True
            except ImportError:
                # pysnmp entity API not available, try subprocess
                self.logger.info("pysnmp entity API not available, using snmpset command")
                return self._send_snmp_reset_via_subprocess(host, reset_oid, ACTION_EXECUTE, community, port)
            except Exception as e:
                self.logger.warning(f"Failed to use pysnmp entity API: {e}, trying subprocess fallback")
                return self._send_snmp_reset_via_subprocess(host, reset_oid, ACTION_EXECUTE, community, port)
        except Exception as e:
            self.logger.error(f"Exception sending SNMP reset command: {e}")
            import traceback
            self.logger.debug(traceback.format_exc())
            return False
    
    def _send_snmp_reset_via_subprocess(self, host: str, oid: str, value: int, community: str, port: int) -> bool:
        """
        Send SNMP SET command using subprocess (snmpset command) as fallback.
        ...
        try:
            ...
            self.logger.info(f"Using snmpset command: {' '.join(cmd)}")
            result = subprocess.run(
                cmd,
                capture_output=True,
                text=True,
            ...
            if result.returncode == 0:
                self.logger.info(f"SNMP reset command sent successfully via snmpset: {result.stdout.strip()}")
                return True
            else:
                self.logger.error(f"snmpset failed with return code {result.returncode}")
                if result.stderr:
                    self.logger.error(f"Error: {result.stderr.strip()}")
                return False
        except FileNotFoundError:
            self.logger.error("snmpset command not found. Please install net-snmp:")
            self.logger.error("  Ubuntu/Debian: sudo apt-get install snmp")
            return False
        except subprocess.TimeoutExpired:
            self.logger.error("snmpset command timed out")
            return False
        except Exception as e:
            self.logger.error(f"Exception using snmpset: {e}")
            return False
    
    def _perform_reset(self):
        """
        Perform reset operation when reset button (GPIO 21) is pressed.
        This function:
        1. Plays buzzer sound for 10 seconds
        2. Turns off all LEDs
        3. Turns on LED 10
        4. Sends SNMP reset command to the STS device
        """
        try:
            self.logger.info("=" * 70)
            self.logger.info("RESET BUTTON PRESSED (GPIO 21) - Starting reset sequence")
            self.logger.info("=" * 70)
            
            # Step 1: Play buzzer for 10 seconds
            self.logger.info("Step 1: Playing buzzer for 10 seconds...")
            if self.panel_led_controller:
                try:
                    # Enable buzzer with continuous beep pattern
                    if hasattr(self.panel_led_controller, 'enable_buzzer'):
                        self.panel_led_controller.enable_buzzer(
                            continuous=True,
                            beep_pattern=True,
                        ...
                        self.logger.info("Buzzer enabled with beep pattern (volume: 75%)")
                        
                        # Wait for 10 seconds while buzzer plays
                        time.sleep(10.0)
                        
                        # Stop buzzer after 10 seconds
                        if hasattr(self.panel_led_controller, 'disable_buzzer'):
                            self.panel_led_controller.disable_buzzer()
                            self.logger.info("Buzzer stopped after 10 seconds")
                    else:
                        self.logger.warning("Panel LED controller does not have enable_buzzer method")
                        # If buzzer not available, still wait 10 seconds
                        time.sleep(10.0)
                except Exception as e:
                    self.logger.error(f"Error with buzzer: {e}")
                    # Continue with reset sequence even if buzzer fails
            else:
                self.logger.warning("Panel LED controller not available - cannot play buzzer")
                # Wait 10 seconds anyway to maintain timing
                time.sleep(10.0)
            
            # Step 2: Turn off all LEDs
            self.logger.info("Step 2: Turning off all LEDs...")
            if self.panel_led_controller:
                try:
                    disabled_green = self.panel_led_controller.disable_all_green_leds()
                    disabled_red = self.panel_led_controller.disable_all_red_leds()
                    self.logger.info(f"Disabled {disabled_green} green LEDs and {disabled_red} red LEDs")
                except Exception as e:
                    self.logger.error(f"Error disabling LEDs: {e}")
            else:
                self.logger.warning("Panel LED controller not available - cannot disable LEDs")
            
            # Step 3: Turn on LED 10
            self.logger.info("Step 3: Turning on LED 10...")
            if self.panel_led_controller:
                try:
                    if self.panel_led_controller.enable_led(10):
                        self.logger.info("LED 10 enabled successfully")
                    else:
                        self.logger.warning("Failed to enable LED 10")
                except Exception as e:
                    self.logger.error(f"Error enabling LED 10: {e}")
            else:
                self.logger.warning("Panel LED controller not available - cannot enable LED 10")
            
            # Step 4: Send SNMP reset command to STS device
            self.logger.info("Step 4: Sending SNMP reset command to STS device...")
            if self.ups_host:
                reset_success = self._send_snmp_reset_command(
                    host=self.ups_host,
                    community=self.snmp_community,
                    port=self.snmp_port
                )
                if reset_success:
                    self.logger.info(f"SNMP reset command sent successfully to {self.ups_host}")
                    self.logger.info("STS device should reset all parameters to default values")
                else:
                    self.logger.error(f"Failed to send SNMP reset command to {self.ups_host}")
            else:
                self.logger.error("UPS host not configured - cannot send reset command")
                self.logger.error("Please configure UPS_IP in config.py or set allowed_ips")
            
            self.logger.info("=" * 70)
            self.logger.info("Reset sequence completed")
            self.logger.info("=" * 70)
            
        except Exception as e:
            self.logger.error(f"Error in reset sequence: {e}")
            import traceback
            self.logger.error(traceback.format_exc())
    
    def _mute_button_monitor_thread(self):
        """Background thread to monitor mute button (event-driven with GPIO.BOTH)."""
        if not RPI_GPIO_AVAILABLE:
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"[BUTTON] GPIO {self.mute_button_pin} configured as input with pull-up")
            except Exception as e:
                error_msg = str(e)
                # Check if it's a "GPIO busy" error
                if 'busy' in error_msg.lower() or 'lgpio.error' in error_msg or 'GPIO busy' in error_msg:
                    self.logger.error(f"[BUTTON] GPIO {self.mute_button_pin} is BUSY - another process may be using it")
                    self.logger.error(f"[BUTTON] Error details: {e}")
                    self.logger.warning(f"[BUTTON] Attempting to recover by checking if pin is already configured...")
                    
                    # Try to read the pin to see if it's already configured
                    try:
                        current_state = GPIO.input(self.mute_button_pin)
                        self.logger.info(f"[BUTTON] GPIO {self.mute_button_pin} is readable (state={current_state}) - pin may already be configured")
                        self.logger.warning(f"[BUTTON] Continuing with existing GPIO configuration - button monitoring may not work correctly")
                        self.logger.warning(f"[BUTTON] SOLUTION: Stop other processes using GPIO {self.mute_button_pin} or restart the service")
                        # Don't raise - continue with existing configuration
                    except Exception as read_error:
                        self.logger.error(f"[BUTTON] Cannot read GPIO {self.mute_button_pin}: {read_error}")
                        self.logger.error(f"[BUTTON] Mute button monitoring DISABLED - GPIO {self.mute_button_pin} is not accessible")
                        raise  # Re-raise if we can't even read the pin
                else:
                    self.logger.error(f"[BUTTON] Failed to setup GPIO {self.mute_button_pin} as input: {e}", exc_info=True)
            ...
            self.logger.info(f"[BUTTON] Initial button state: {self.mute_button_last_state} (LOW={GPIO.LOW}, HIGH={GPIO.HIGH})")
            ...
            # Use lambda directly like test program to ensure proper callback binding
            # The lambda captures 'self' in a closure, making it work like the test program's lambda
            GPIO.add_event_detect(
                self.mute_button_pin,
                GPIO.BOTH,  # Detect both rising and falling edges (same as test program)
                callback=lambda channel: self._mute_button_callback(channel),  # Lambda like test program
                bouncetime=int(self.mute_button_debounce_time * 1000)  # Convert to milliseconds (10ms, same as test program)
            )
            ...
            self.logger.info(f"[BUTTON] Starting polling loop for GPIO {self.mute_button_pin} (poll_interval={poll_interval}s, initial_state={last_state})")
            
            # Track if event detection is still active (for conflict detection)
            last_event_check_time = time.time()
            event_check_interval = 5.0  # Check every 5 seconds if event detection is still active
            
            while self.mute_button_running and not self._shutdown_requested:
                time.sleep(poll_interval)
                
                # Check periodically if event detection is still active (detect conflicts with other scripts)
                current_time = time.time()
                if current_time - last_event_check_time >= event_check_interval:
                    last_event_check_time = current_time
                    # Note: RPi.GPIO doesn't provide a direct way to check if event detection is active
                    # But we can detect if another process removed it by checking if we can still read the pin
                    try:
                        test_read = GPIO.input(self.mute_button_pin)
                        # If we can read, the pin is still configured (good)
                    except Exception as e:
                        self.logger.warning(f"[BUTTON] GPIO {self.mute_button_pin} may have been reconfigured by another process: {e}")
                        # Try to re-setup the pin
                        try:
                            GPIO.setup(self.mute_button_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
                            # Re-register event detection
                            GPIO.add_event_detect(
                                self.mute_button_pin,
                                GPIO.BOTH,
                                callback=lambda channel: self._mute_button_callback(channel),
                                bouncetime=int(self.mute_button_debounce_time * 1000)
                            )
                            self.logger.info(f"[BUTTON] Re-registered event detection on GPIO {self.mute_button_pin} after conflict")
                        except Exception as reinit_error:
                            self.logger.error(f"[BUTTON] Failed to re-initialize GPIO {self.mute_button_pin}: {reinit_error}")
                ...
                try:
                    ...
                    # Use the most common value (mode) to filter out noise
                    current_state = max(set(rapid_reads), key=rapid_reads.count)
                    is_pressed_now = (current_state == GPIO.LOW)
                    
                    # CRITICAL DEBUGGING: Log if button appears pressed or state is unstable
                    if current_state == GPIO.LOW:
                        self.logger.warning(f"[BUTTON-DEBUG] GPIO {self.mute_button_pin} read as LOW (pressed)! State={current_state}, rapid_reads={rapid_reads}, last_state={last_state}")
                        # Force immediate flush
                        for handler in self.logger.handlers:
                            if isinstance(handler, logging.FileHandler):
                                handler.flush()
                    elif len(set(rapid_reads)) > 1:
                        # State is unstable - might be button bouncing or brief press
                        self.logger.warning(f"[BUTTON-DEBUG] GPIO {self.mute_button_pin} state unstable! rapid_reads={rapid_reads}, using mode={current_state}, last_state={last_state}")
                        # Force immediate flush
                        for handler in self.logger.handlers:
                            if isinstance(handler, logging.FileHandler):
                                handler.flush()
                    ...
                    if current_state != last_state:
                        ...
                        self.logger.info(f"[BUTTON] Polling fallback detected state change on GPIO {self.mute_button_pin}: {last_state} -> {current_state} (pressed={is_pressed_now}, consecutive_same={consecutive_same_state_count})")
                        # Force immediate flush to log file for button events
                        for handler in self.logger.handlers:
                            if isinstance(handler, logging.FileHandler):
                                handler.flush()
                        ...
                        try:
                            self.logger.info(f"[BUTTON] Calling mute button callback directly ({self.mute_button_pin})...")
                            for handler in self.logger.handlers:
                                if isinstance(handler, logging.FileHandler):
                                    handler.flush()
                            self._mute_button_callback(self.mute_button_pin)
                            self.logger.info(f"[BUTTON] Mute button callback completed")
                    ...
                    if current_time - last_poll_time >= 5.0:
                        self.logger.info(f"[BUTTON] Periodic check - GPIO {self.mute_button_pin}: state={current_state} (pressed={is_pressed_now}), last_state={last_state}, unchanged_count={consecutive_same_state_count}")
                        # Also verify GPIO is still configured correctly
                        try:
                            # Try to read pin again to verify it's working
                            verify_read = GPIO.input(self.mute_button_pin)
            ...
            self.logger.error(f"[BUTTON] Error in mute button monitor thread: {e}", exc_info=True)
            import traceback
            self.logger.error(f"[BUTTON] Traceback: {traceback.format_exc()}")
        finally:
            try:
                if RPI_GPIO_AVAILABLE:
                    ...
                    # Note: We don't call GPIO.cleanup() here because it affects ALL pins
                    # The main stop() method will handle full cleanup
            except:
                pass
            self.logger.info("[BUTTON] Mute button monitoring stopped")
    ...
    def _reset_button_monitor_thread(self):
        ...
        try:
            # IMPORTANT: Match test_mute_reset_buttons.py initialization sequence exactly
            # Step 1: Cleanup any existing GPIO state first (like test program)
            try:
            ...
            # Step 2: Reset callback state tracking (same as test program)
            self.reset_button_last_callback_time = 0
            self.reset_button_last_callback_state = None
            
            # Step 3: Setup GPIO mode and pin (match test program sequence)
            # CRITICAL: GPIO mode must be BCM for button pins 19 and 21 to work correctly
            # In BOARD mode, pin 19 = physical pin 19 (BCM GPIO 10), pin 21 = physical pin 21 (BCM GPIO 9)
            # We need BCM GPIO 19 (physical pin 35) and GPIO 21 (physical pin 40)
            if current_gpio_mode is None:
                try:
                    ...
                    # GPIO mode already set - this is OK (same as test program)
                    self.logger.debug(f"[BUTTON] GPIO mode already set: {e}")
                    try:
                        current_gpio_mode = GPIO.getmode()
                    except:
                        current_gpio_mode = GPIO.BCM  # Assume BCM if we can't read it
            elif current_gpio_mode == GPIO.BOARD:
                # GPIO mode is BOARD - this is a CRITICAL problem!
                # We need to convert pin numbers or fail
                self.logger.error(f"[BUTTON] CRITICAL: GPIO mode is BOARD, but button pins are configured for BCM mode!")
            ...
            # Step 4: Setup GPIO pin as input with pull-up (match test program exactly)
            # When button is pressed, pin goes LOW (0)
            # When button is released, pin goes HIGH (1) due to pull-up
            # IMPORTANT: Check if pin is already configured as output (conflict with panel_led_controller)
            try:
                # Try to read pin first to see if it's accessible
                test_read_before = GPIO.input(self.reset_button_pin)
                self.logger.info(f"[BUTTON] GPIO {self.reset_button_pin} read before setup: {test_read_before}")
            except Exception as read_err:
                self.logger.warning(f"[BUTTON] Could not read GPIO {self.reset_button_pin} before setup: {read_err}")
            ...
            try:
                ...
                # Verify pin is now readable as input
                test_read_after = GPIO.input(self.reset_button_pin)
                self.logger.info(f"[BUTTON] GPIO {self.reset_button_pin} read after setup: {test_read_after} (should be {GPIO.HIGH} if button not pressed)")
            except Exception as e:
                error_msg = str(e)
                # Check if it's a "GPIO busy" error
                if 'busy' in error_msg.lower() or 'lgpio.error' in error_msg or 'GPIO busy' in error_msg:
                    self.logger.error(f"[BUTTON] GPIO {self.reset_button_pin} is BUSY - another process may be using it")
                    self.logger.error(f"[BUTTON] Error details: {e}")
                    self.logger.warning(f"[BUTTON] Attempting to recover by checking if pin is already configured...")
                    
                    # Try to read the pin to see if it's already configured
                    try:
                        current_state = GPIO.input(self.reset_button_pin)
                        self.logger.info(f"[BUTTON] GPIO {self.reset_button_pin} is readable (state={current_state}) - pin may already be configured")
                        self.logger.warning(f"[BUTTON] Continuing with existing GPIO configuration - button monitoring may not work correctly")
                        self.logger.warning(f"[BUTTON] SOLUTION: Stop other processes using GPIO {self.reset_button_pin} or restart the service")
                        # Don't raise - continue with existing configuration
                    except Exception as read_error:
                        self.logger.error(f"[BUTTON] Cannot read GPIO {self.reset_button_pin}: {read_error}")
                        self.logger.error(f"[BUTTON] Reset button monitoring DISABLED - GPIO {self.reset_button_pin} is not accessible")
                        raise  # Re-raise if we can't even read the pin
                else:
                    self.logger.error(f"[BUTTON] Failed to setup GPIO {self.reset_button_pin} as input: {e}", exc_info=True)
                    raise  # Re-raise to stop thread initialization
            
            # Step 5: Initialize button state to current physical state (match test program exactly)
            ...
            # Set to past time to ensure first state change is always processed (same as test program)
            ...
            # Step 6: Setup interrupt callback with debouncing (match test program exactly)
            # Use GPIO.BOTH to detect both rising and falling edges (event-driven approach)
            # Use lambda directly like test program to ensure proper callback binding
            # The lambda captures 'self' in a closure, making it work like the test program's lambda
            GPIO.add_event_detect(
                self.reset_button_pin,
                GPIO.BOTH,  # Detect both rising and falling edges (same as test program)
                callback=lambda channel: self._reset_button_callback(channel),  # Lambda like test program
                bouncetime=int(self.reset_button_debounce_time * 1000)  # Convert to milliseconds (10ms, same as test program)
            )
            
            # Verify event detection was registered (match test program)
            try:
                ...
                self.logger.info(f"[BUTTON] Event detection registered - current pin state: {pin_state} (LOW={GPIO.LOW}, HIGH={GPIO.HIGH})")
                self.logger.info(f"[BUTTON] Interrupt handler registered successfully (matches test_mute_reset_buttons.py)")
            except Exception as e:
                self.logger.warning(f"[BUTTON] Could not verify event detection registration: {e}")
            ...
            while self.reset_button_running and not self._shutdown_requested:
                ...
                if current_time - last_event_check_time >= event_check_interval:
                    ...
                    try:
                        ...
                        self.logger.warning(f"[BUTTON] GPIO {self.reset_button_pin} may have been reconfigured by another process: {e}")
                        # Try to re-setup the pin
                        try:
                            GPIO.setup(self.reset_button_pin, GPIO.IN, pull_up_down=GPIO.PUD_UP)
                            # Re-register event detection
                            GPIO.add_event_detect(
                                self.reset_button_pin,
                                GPIO.BOTH,
                                callback=lambda channel: self._reset_button_callback(channel),
                                bouncetime=int(self.reset_button_debounce_time * 1000)
                            )
                            self.logger.info(f"[BUTTON] Re-registered event detection on GPIO {self.reset_button_pin} after conflict")
                        except Exception as reinit_error:
                            self.logger.error(f"[BUTTON] Failed to re-initialize GPIO {self.reset_button_pin}: {reinit_error}")
                
                # PRIMARY METHOD: Use polling to detect button presses (more reliable than callbacks in threads)
                # This matches the test script's polling mode approach
                try:
                    # Read GPIO pin multiple times rapidly to catch brief button presses
                    # This helps catch very brief presses that might be missed by single reads
                    rapid_reads = []
                    ...
                    # CRITICAL DEBUGGING: Log EVERY read when button might be pressed
                    # This will help us see if the button is actually going LOW
                    if current_state == GPIO.LOW:
                        # Button appears to be pressed - log immediately with full details
                        self.logger.warning(f"[BUTTON-DEBUG] GPIO {self.reset_button_pin} read as LOW (pressed)! State={current_state}, LOW={GPIO.LOW}, HIGH={GPIO.HIGH}, last_state={last_state}, consecutive={consecutive_same_state_count}, rapid_reads={rapid_reads}")
                        # Force immediate flush
                        for handler in self.logger.handlers:
                            if isinstance(handler, logging.FileHandler):
                                handler.flush()
                        ...
                        # State is unstable - might be button bouncing or brief press
                        self.logger.warning(f"[BUTTON-DEBUG] GPIO {self.reset_button_pin} state unstable! rapid_reads={rapid_reads}, using mode={current_state}, last_state={last_state}")
                        # Force immediate flush
                        for handler in self.logger.handlers:
                            if isinstance(handler, logging.FileHandler):
                                handler.flush()
                    
                    # Check if state changed (polling detection - PRIMARY METHOD)
                    if current_state != last_state:
                        # State changed - trigger callback manually (polling fallback)
                        self.logger.info(f"[BUTTON] Polling fallback detected state change on GPIO {self.reset_button_pin}: {last_state} -> {current_state} (LOW={GPIO.LOW}, HIGH={GPIO.HIGH}, pressed={is_pressed_now}, consecutive_same={consecutive_same_state_count})")
                        # Force immediate flush to log file for button events
                        for handler in self.logger.handlers:
                            if isinstance(handler, logging.FileHandler):
                                handler.flush()
                        consecutive_same_state_count = 0
                        try:
                            self.logger.info(f"[BUTTON] Calling reset button callback directly ({self.reset_button_pin})...")
                            for handler in self.logger.handlers:
                                if isinstance(handler, logging.FileHandler):
                                    handler.flush()
                            self._reset_button_callback(self.reset_button_pin)
                            self.logger.info(f"[BUTTON] Reset button callback completed")
                            # Force immediate flush after callback
                            for handler in self.logger.handlers:
                                if isinstance(handler, logging.FileHandler):
                                    handler.flush()
                        ...
                        consecutive_same_state_count += 1
                        # Log every state read when button might be pressed (for debugging)
                        # Only log if we're seeing state=0 (LOW/pressed) to avoid log spam
                        if current_state == GPIO.LOW:
                            # Button is pressed - log every read to track duration
                            if consecutive_same_state_count % 10 == 0 or consecutive_same_state_count == 1:
                                self.logger.info(f"[BUTTON] GPIO {self.reset_button_pin} is LOW (pressed) - consecutive reads: {consecutive_same_state_count}")
                                # Force immediate flush when button is detected as pressed
                                for handler in self.logger.handlers:
                                    if isinstance(handler, logging.FileHandler):
                                        handler.flush()
                            # If button has been pressed for more than 5 reads (50ms), trigger callback
                            # This ensures we catch button presses even if they're very brief
                            if consecutive_same_state_count == 5:
                                self.logger.warning(f"[BUTTON] Button has been LOW for {consecutive_same_state_count * poll_interval:.3f}s - triggering callback to ensure press is detected")
                                try:
                                    self._reset_button_callback(self.reset_button_pin)
                                except Exception as e:
                                    self.logger.error(f"[BUTTON] Error triggering callback for sustained button press: {e}", exc_info=True)
                    
                    # Periodic diagnostic: log state every 5 seconds with more detail
                    current_time = time.time()
                    if current_time - last_poll_time >= 5.0:
                        # Read multiple times to check for any state changes
                        test_reads = [GPIO.input(self.reset_button_pin) for _ in range(5)]
                        state_variations = len(set(test_reads))
                        self.logger.info(f"[BUTTON] Periodic check - GPIO {self.reset_button_pin}: state={current_state} (LOW={GPIO.LOW}, HIGH={GPIO.HIGH}, pressed={is_pressed_now}), last_state={last_state}, unchanged_count={consecutive_same_state_count}")
                        self.logger.info(f"[BUTTON] GPIO {self.reset_button_pin} test reads (5x): {test_reads} (variations={state_variations}, all should be {GPIO.HIGH} if button not pressed)")
                        # Also verify GPIO is still configured correctly
                        try:
                            # Try to read pin again to verify it's working
                            ...
                            if verify_read != current_state:
                                self.logger.warning(f"[BUTTON] GPIO read inconsistency: current_state={current_state}, verify_read={verify_read}")
                            # Check if button might be stuck or wiring issue
                            if current_state == GPIO.LOW:
                                self.logger.warning(f"[BUTTON] GPIO {self.reset_button_pin} is reading LOW - button may be pressed or wiring issue!")
                            elif state_variations > 1:
                                self.logger.warning(f"[BUTTON] GPIO {self.reset_button_pin} state is unstable (variations={state_variations}) - possible wiring issue or button bouncing")
                        except Exception as e:
                            self.logger.error(f"[BUTTON] GPIO read failed during periodic check: {e}")
                        last_poll_time = current_time
        ...
        try:
            ...
            if RPI_GPIO_AVAILABLE and not self.is_windows:
                ...
                try:
                    ...
                    self.mute_button_thread = threading.Thread(target=self._mute_button_monitor_thread, daemon=True)
                    self.mute_button_thread.start()
                    self.logger.info(f"Mute button monitoring thread started (GPIO {self.mute_button_pin})")
                except Exception as e:
    ...
    def stop(self):
        ...
        # IMPORTANT: Do GPIO cleanup BEFORE panel_led_controller.cleanup()
        # This ensures button pins are properly released before panel_led_controller cleans up
        # Final GPIO cleanup - ensure all pins are reset
        # CRITICAL: Properly clean up GPIO pins so other scripts can use them
        if RPI_GPIO_AVAILABLE:
            try:
                self.logger.info("[BUTTON] Starting final GPIO cleanup for button pins...")
                
                # Step 1: Remove event detection from button pins (must be done before GPIO.cleanup())
                if hasattr(self, 'mute_button_pin'):
                    try:
                        GPIO.remove_event_detect(self.mute_button_pin)
                        self.logger.info(f"[BUTTON] Removed event detection from GPIO {self.mute_button_pin} (final cleanup)")
                    except Exception as e:
                        self.logger.debug(f"[BUTTON] Could not remove event detection from GPIO {self.mute_button_pin}: {e}")
                
                if hasattr(self, 'reset_button_pin'):
                    try:
                        GPIO.remove_event_detect(self.reset_button_pin)
                        self.logger.info(f"[BUTTON] Removed event detection from GPIO {self.reset_button_pin} (final cleanup)")
                    except Exception as e:
                        self.logger.debug(f"[BUTTON] Could not remove event detection from GPIO {self.reset_button_pin}: {e}")
                
                # Step 2: Reset GPIO mode to allow other scripts to set their own mode
                # This is important because test_mute_reset_buttons.py needs to set GPIO mode
                try:
                    current_mode = GPIO.getmode()
                    if current_mode is not None:
                        self.logger.info(f"[BUTTON] Current GPIO mode: {current_mode} (BCM={GPIO.BCM}, BOARD={GPIO.BOARD})")
                except:
                    pass
                
                # Step 3: Call GPIO.cleanup() to fully release GPIO resources
                # This ensures other scripts (like test_mute_reset_buttons.py) can use the pins
                # GPIO.cleanup() resets all pins to their default state and clears the mode
                GPIO.cleanup()
                self.logger.info("[BUTTON] GPIO.cleanup() called - all GPIO pins and mode reset for other processes")
                
                # Step 4: Verify cleanup by checking GPIO mode (should be None after cleanup)
                try:
                    mode_after_cleanup = GPIO.getmode()
                    if mode_after_cleanup is None:
                        self.logger.info("[BUTTON] GPIO mode successfully reset to None - ready for other scripts")
                    else:
                        self.logger.warning(f"[BUTTON] GPIO mode still set to {mode_after_cleanup} after cleanup - may cause issues")
                except:
                    pass
                    
            except Exception as e:
                self.logger.warning(f"[BUTTON] Error during final GPIO cleanup: {e}", exc_info=True)
                # Try to cleanup anyway - don't let cleanup errors prevent shutdown
                try:
                    GPIO.cleanup()
                    self.logger.info("[BUTTON] GPIO.cleanup() called in exception handler")
                except Exception as cleanup_error:
                    self.logger.error(f"[BUTTON] Failed to call GPIO.cleanup() in exception handler: {cleanup_error}")
        
        # Cleanup Panel LED Controller (after GPIO cleanup to avoid conflicts)
        if self.panel_led_controller:
            try:
                # Note: panel_led_controller.cleanup() will also call GPIO.cleanup()
                # but that's OK - GPIO.cleanup() is safe to call multiple times
