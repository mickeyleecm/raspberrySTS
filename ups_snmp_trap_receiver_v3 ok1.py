            ...
            while self.mute_button_running and not self._shutdown_requested:
                ...
                try:
                    ...
                    if current_time - last_poll_time >= 5.0:
                        self.logger.info(f"[BUTTON] Periodic check - GPIO {self.mute_button_pin} (Mute): state={current_state} (pressed={is_pressed_now}), last_state={last_state}, unchanged_count={consecutive_same_state_count}")
        ...
        try:
            ...
            if RPI_GPIO_AVAILABLE and not self.is_windows:
                ...
                try:
                    ...
                    self.logger.info(f"Button monitoring thread started (Mute: GPIO {self.mute_button_pin}, Reset: GPIO {self.reset_button_pin})")
                except Exception as e:
