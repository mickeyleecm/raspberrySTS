                ...
                if input_thread.is_alive():
                    ...
                    if self.panel_led_controller:
                        try:
                            ...
                            # Enable LED 10 (alarm LED) if alarm LED is enabled
                            if self.alarm_led_enabled:
                                self.panel_led_controller.enable_led(10)
                                ...
                                self.logger.info(f"TIMEOUT detected - Disabled LEDs {leds_to_disable}, Alarm LED disabled (LED 10 not enabled)")
                        except Exception as e:
                ...
                try:
                    ...
                    if (source_a_status_lower == 'fail' and source_b_status_lower == 'fail'):
                        ...
                        if self.alarm_led_enabled:
                            ...
                            self.logger.info(f"Both sources fail - Disabled LEDs {leds_to_disable}, Alarm LED disabled (LED 10 not enabled)")
                    else:
    ...
    def _mute_button_monitor_thread(self):
        ...
        try:
            ...
            try:
                ...
                self.logger.info(f"[BUTTON] GPIO mode set to BCM")
            ...
            # Use lambda wrapper like test program to ensure proper binding
            GPIO.add_event_detect(
                self.mute_button_pin,
                GPIO.BOTH,  # Detect both rising and falling edges (event-driven)
                callback=lambda channel: self._mute_button_callback(channel),  # Lambda wrapper like test program
            ...
            # Keep thread alive and periodically check button state for diagnostics
            last_diag_time = time.time()
            while self.mute_button_running and not self._shutdown_requested:
                time.sleep(0.1)
                
                # Periodic diagnostic: check button state every 5 seconds
                current_time = time.time()
                if current_time - last_diag_time >= 5.0:
                    try:
                        current_state = GPIO.input(self.mute_button_pin)
                        is_pressed_now = (current_state == GPIO.LOW)
                        self.logger.debug(f"[BUTTON] Periodic check - GPIO {self.mute_button_pin} state: {current_state} (pressed={is_pressed_now})")
                        last_diag_time = current_time
                    except Exception as e:
                        self.logger.warning(f"[BUTTON] Error in periodic button check: {e}")
    ...
    def _reset_button_monitor_thread(self):
        ...
        try:
            ...
            GPIO.add_event_detect(
                ...
                GPIO.BOTH,  # Detect both rising and falling edges (event-driven)
                callback=lambda channel: self._reset_button_callback(channel),  # Lambda wrapper like test program
            ...
            while self.reset_button_running and not self._shutdown_requested:
                ...
                if current_time - last_diag_time >= 5.0:
                    try:
                        ...
                        is_pressed_now = (current_state == GPIO.LOW)
                        self.logger.debug(f"[BUTTON] Periodic check - GPIO {self.reset_button_pin} state: {current_state} (pressed={is_pressed_now})")
                        last_diag_time = current_time
                    except Exception as e:
                        self.logger.warning(f"[BUTTON] Error in periodic button check: {e}")
